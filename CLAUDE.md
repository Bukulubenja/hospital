...
`hospital.User` is a custom `AbstractUser` (`AUTH_USER_MODEL = 'hospital.User'`) with a `role` field (`ADMIN`, `DOCTOR`, `NURSE`, `RECEPTIONIST`, `LAB`, `PHARMACIST`, `CASHIER`, `PATIENT`, `STOCK_MANAGER`). There is no separate permissions/groups system — role gating is done in application code:

- `hospital/decorators.py`: `@role_required("DOCTOR")` (wraps `login_required`) restricts a view to specific roles.
- `hospital/permissions.py`: object-level checks beyond role, e.g. `can_doctor_access(user, visit)` requires the visit be *assigned* to that doctor, not just that the user has the DOCTOR role — don't loosen this without a reason; it was tightened deliberately after a cross-patient-access gap was found.
...
- `hospital/signals.py`: `post_save` on `User` auto-creates a `Doctor`/`Nurse` profile row via `transaction.on_commit`. This is wired up in `hospital/apps.py`'s `ready()` — if you ever see the signal not firing, check that import is still there.
...
Each staff role's workflow (Reception, Doctor, Pharmacy, Lab) follows the same shape — replicate it for new roles rather than inventing a new one:
...
- `static/css/reception.css` — shared "light admin panel" theme used by Reception, Pharmacy, and Lab pages (badges, cards, tables, forms). Reuse this for new admin-style staff pages rather than writing a new stylesheet.
...
`hospital/tests.py` has one `TestCase` class per workflow (`ReceptionWorkflowTests`, `DoctorWorkflowTests`, `PharmacyWorkflowTests`, `LabWorkflowTests`). Tests exercise views through `self.client` (POST/GET + assert redirects/DB state), not by calling service functions directly — this is what catches things like missing form fields or CSRF/permission wiring. Tests run against a real throwaway Postgres database created per run, not SQLite or mocks.

## Current workflow status

Built end-to-end (views + forms + templates + tests): **Reception** (register patient, book appointment, check in → queue ticket), **Doctor** (consult, vitals, diagnosis, prescribe, order labs, complete visit), **Pharmacy** (dispense prescriptions with FEFO stock deduction), **Lab** (record results, route visit onward).

Still stubs (route exists, template renders, no real logic wired up): Cashier/billing (`VisitInvoice`/`Payment` models exist but nothing generates or collects on them), Nurse, Admin, Stock Manager dashboards, and the telemedicine/records-download/prescription-refill patient-facing views.
